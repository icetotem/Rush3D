...
static const unsigned int DT_RUSH_BITS = 20;
...
public:
	...
	{
#ifdef DT_POLYREF64
		return ((dtPolyRef)salt << (DT_RUSH_BITS+DT_TILE_BITS)) | ((dtPolyRef)it << DT_RUSH_BITS) | (dtPolyRef)ip;
#else
	...
	inline void decodePolyId(dtPolyRef ref, unsigned int& salt, unsigned int& it, unsigned int& ip) const
	{
		...
		const dtPolyRef polyMask = ((dtPolyRef)1<<DT_RUSH_BITS)-1;
		salt = (unsigned int)((ref >> (DT_RUSH_BITS+DT_TILE_BITS)) & saltMask);
		it = (unsigned int)((ref >> DT_RUSH_BITS) & tileMask);
...
#ifdef DT_POLYREF64
		const dtPolyRef saltMask = ((dtPolyRef)1<<DT_SALT_BITS)-1;
		return (unsigned int)((ref >> (DT_RUSH_BITS+DT_TILE_BITS)) & saltMask);
#else
		const dtPolyRef saltMask = ((dtPolyRef)1<<m_saltBits)-1;
...
#ifdef DT_POLYREF64
		const dtPolyRef tileMask = ((dtPolyRef)1<<DT_TILE_BITS)-1;
		return (unsigned int)((ref >> DT_RUSH_BITS) & tileMask);
#else
		const dtPolyRef tileMask = ((dtPolyRef)1<<m_tileBits)-1;
